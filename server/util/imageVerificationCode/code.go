...
// 生成图片和坐标
func Create() (*system.VerificationCodeDao, error) {
	return jigsaw.Create()
}

func (j *Jigsaw) Create() (*system.VerificationCodeDao, error) {
	if j.realBgNums == 0 {
		return nil, errors.New("no valid image found")
	}
	...
	img = fmt.Sprintf("url(\"data:image/png;base64,%s\")", img)
	bg = fmt.Sprintf("url(\"data:image/png;base64,%s\")", bg)
	imageRandYS := fmt.Sprintf("%dpx", imageRandY)
	return &system.VerificationCodeDao{ImageBase: bg, BgBase: img, X: imageRandX, Y: imageRandYS}, nil
}
